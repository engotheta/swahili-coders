					...
					<div class="row align-items-center">
							<div class="col-md-7">
									<div class="page-header-title">
											<h5 class="m-b-10">{{ $selected_category->title}}'s communities</h5>
									</div>
									<ul class="breadcrumb">
											<li class="breadcrumb-item"><a href="{{ url('/cms') }}"><i class="feather icon-home"></i></a></li>
											<li class="breadcrumb-item"><a href="javascript:">Site Contents</a></li>
											<li class="breadcrumb-item"><a href="{{ url('/cms/community-categories') }}">communities</a></li>
	...
	<div class="main-body">
			<div class="page-wrapper">
				...
				<div class="row">
					<div class="col-md-3">
						<div class="row">
							<div class="col-12">
								<a href="{{ URL::route('cms.community-categories.create') }}" data-toggle="modal" data-target="#newCategory" class="btn btn-outline-primary col-12 ">
									...
									<table class="table table-hover">
										...
										<tbody>
											...
											@foreach($categories as $key => $category)
												<tr class="hover-show">
													<td>
														<a class=" {{($category->active ==1)? 'text-dark': 'text-danger'}}" href="{{ URL::route('cms.community-categories.show', $category->slug) }}">

																	{{ $category->title_en }} <br>
																	<small>{{ number_format(count($category->communities)) }} Entities</small>
																	<a href="{{ URL::route('cms.community-categories.edit', $category->id) }}"
																		data-toggle="modal" data-target="#OpenModelContainer{{ $key }}"
																		class="px-1 show-on-hover" >
																		<i class="feather icon-edit"></i>
																	</a>
																	<a href="{{ URL::route('cms.community-categories.destroy', $category->id) }}"
									...
									<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
											<div class="modal-content">
													...
													<div class="modal-body">
														{!! Form::model($category, ['route' => ['cms.community-categories.update', $category->id], 'method' => 'PATCH' ,'class'=>'loading-form']) !!}
					...
					<div class="col-md-9 ">
						<div class="card " >
							...
							<div class="card-body position-relative" style="overflow:auto;height:70vh">
								...
								@if($selected_category->communities->count() == 0)
									<div class="col-md-12">
										<div class="alert alert-primary" role="alert">{{ label('lbl_no_information') }}	</div>
									</div>
								@else
									...
									@foreach($selected_category->communities as $key => $entity)
										<tr>
											<td>{{ $key + 1}}</td>
											<td>
												@empty ($entity->photo_url)
													<img loading="lazy" src="{{ asset('/site/images/default.jpg') }}" style="max-height:40px !important;">
												@else
													<img loading="lazy" src="{{ asset($entity->photo_url) }}" style="max-height:40px !important;">
												@endempty
											</td>
											...
											<td>
												<a href="{{ URL::route('cms.communities.edit', $entity->id) }}" data-toggle="modal" data-target="#OpenDocumentModel{{ $key }}" class="label theme-bg2 text-white f-12">View & Edit</a>
												{!! link_to_route('cms.communities.destroy', "Delete", array($entity->id), array('data-method' => 'delete', 'data-confirm' => 'Are you Sure' ,'class' => 'label theme-b bg-danger text-white f-12')) !!}
											</td>
													...
													<div class="modal-dialog modal-dialog-centered modal-lg" role="document" style="max-width:680px">
															<div class="modal-content">
																	...
																	<div class="modal-body">
																		<div class="row">
																			<div class="col-md-12">
																				{!! Form::model($entity, ['route' => ['cms.communities.update', $entity->id],'files'=> true,
																				'method' => 'PATCH' ,'class'=>'loading-form']) !!}
																					{{-- @include('cms.includes.cropimage',['img' => asset('/uploads/communities/'.$entity->photo_url)]) --}}
																					@include('cms.community-categories._community_form', ['submitButton' => "Update"])
																				{!! Form::close() !!}
																			</div>
																		</div>
																	</div>
			...
			<div class="modal fade" id="newCategory" tabindex="-1" role="dialog" aria-labelledby="newCategory" aria-hidden="true">
					<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
							<div class="modal-content">
									<div class="modal-header">
											<h5 class="modal-title" id="exampleModalLongTitle"> <b>New Community Category</b> </h5>
											<button type="button" class="close" data-dismiss="modal" aria-label="Close">
													<span aria-hidden="true">×</span>
											</button>
									</div>
									<div class="modal-body">
										{!! Form::open(['route' => 'cms.community-categories.index' ,'class'=>'loading-form']) !!}
					...
					<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
							<div class="modal-content">
									...
									<div class="modal-body px-3">
										<form action="{{url('cms/communities-upload')}}" id="uploadEntityForm" method="POST" enctype="multipart/form-data">
												@csrf
			...
			<div class="modal fade" id="newDocument" tabindex="-1" role="dialog" aria-labelledby="newDocument" aria-hidden="true">
					<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
							<div class="modal-content">
									...
									<div class="modal-body px-3">
										{!! Form::open(['route' => 'cms.communities.index','files'=>true ,'class'=>'loading-form']) !!}
											{{-- @include('cms.includes.cropimage',['img' => ""]) --}}
											@include('cms.community-categories._community_form', ['submitButton' => "Save"])
										{!! Form::close() !!}
									</div>
							</div>
					</div>
