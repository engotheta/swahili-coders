<ul class="nav site-inner-navbar">

    {{-- @if(permission('*. .*')) --}}
      <li class="nav-item site-menu-caption">
          <label>Analytics</label>
      </li>

      <li data-username="dashboard Default cms" class="nav-item @php activeLink(['cms']) @endphp">
          <a href="{{ url('/cms')}}" class="nav-link ">
            <span class="site-micon"><i class="feather icon-home"></i></span>
            <span class="site-mtext">Dashboard</span>
          </a>
      </li>
    {{-- @endif --}}
    ...
    <li data-username="backgrounds sliders news events galleries gallery photos videos audios media" class="nav-item site-hasmenu
      @php activeLink(['backgrounds','sliders','news','galleries','media','events']) @endphp">
        <a href="javascript:" class="nav-link ">
          <span class="site-micon"><i class="fas fa-camera-retro"></i></span><span class="site-mtext">Media Center</span></a>
        <ul class="site-submenu">
          ...
          @if(permission('*.backgrounds.*'))
            <li class="@php activeLink(['backgrounds']) @endphp"><a href="{{ url('cms/backgrounds') }}"> Inner Page Background</a></li>
          @endif
    ...
    <li data-username="pages" class="nav-item @php activeLink(['pages']) @endphp">
        <a href="{{ url('cms/pages') }}" class="nav-link">
          <span class="site-micon"><i class="feather icon-copy"></i></span>
          <span class="site-mtext">Pages</span>
        </a>
    </li>
    @endif
    ...
    <li data-username="documents" class="nav-item @php activeLink(['document-categories','documents']) @endphp">
        <a href="{{ url('/cms/document-categories') }}" class="nav-link">
          <span class="site-micon"><i class="feather icon-file-text"></i></span>
          <span class="site-mtext">Publications</span>
        </a>
    </li>
    @endif
     --}}

    @if(permission(['*.online_services.*','*.more-infos.*','*.faqs.*','*.hospitals.*','*.passengers.*','*.partners.*','*.projects.*','*.communties.*',
    '*.services.*','*.trainings.*','*.archievements.*','*.product_categories.*','*.products.*','*.warehouses.*','*.operators.*','*.colateral_managers.*','*.online_services.*','*.howdois.*','*.howdois.*']))
     <li data-username="online services passengers information corner how do i partners"
         class="nav-item site-hasmenu @php activeLink(['online_services','more-infos','achievements','partners','passengers','operators','communities','community-categories','projects','services','warehouses','colateral_managers','trainings','products','product_categories']) @endphp">
         <a href="javascript:" class="nav-link">
           <span class="site-micon"><i class="fa fa-cart-plus"></i></span><span class="site-mtext">{{label('lbl_site_title_short')}} Info</span></a>
         <ul class="site-submenu">
          ...
          @if(permission('*.projects.*'))
             <li class="@php activeLink(['projects']) @endphp"><a href="{{url('/cms/projects')}}"> Projects</a></li>
           @endif

           @if(permission('*.communities.*','*.community-categories.*'))
             <li class="@php activeLink(['communities']) @endphp"><a href="{{url('/cms/community-categories')}}"> Communities</a></li>
           @endif
          ...
          {{-- @if(permission('*.licensed-entities.*','*.licensed-entity-categories.*'))
            <li data-username="licensed-entities" class="nav-item @php activeLink(['licensed-entity-categories','licensed-entities']) @endphp">
                <a href="{{ url('/cms/licensed-entity-categories') }}" class="nav-link">
                  <span class="site-mtext">Registered Entities</span>
                </a>
            </li>
          @endif --}}
   ...
   @if(permission(['*.organogram.*','*.administration.*','*.administration_categories.*','*.administration_categories_members.*']))
     <li data-username="organogram administration management members" class="nav-item site-hasmenu @php activeLink(['organogram','administration','administration_categories','administration_categories_members']) @endphp">
         <a href="javascript:" class="nav-link ">
           <span class="site-micon"><i class="feather icon-users"></i></span>
           <span class="site-mtext"> Members</span>
         </a>
         <ul class="site-submenu">
           ...
           <li class="@php activeLink(['administration']) @endphp"><a href="{{ url('cms/administration') }}"> Members</a></li>
           @endif
    ...
    <li data-username="menu menu-items social_links  related_links quick_links support_links " class="nav-item site-hasmenu
      ...
      'about_links','related_links','home_links','media_links']) @endphp">
        <a href="javascript:" class="nav-link ">
          <span class="site-micon"><i class="feather icon-link-2"></i></span>
          <span class="site-mtext">Navigation</span>
        </a>
        <ul class="site-submenu">
          ...
          <li data-username="menu menu-items" class="nav-item @php activeLink(['menu','menu-items']) @endphp">
            <a href="{{ url('cms/menu') }}" class="nav-link">
              <span class="site-mtext">Menu</span>
            </a>
          </li>
          @endif
    ...
    <li data-username="howdois faqs complaints " class="nav-item site-hasmenu @php activeLink(['howdois','faqs','complaints']) @endphp">
        <a href="javascript:" class="nav-link ">
          <span class="site-micon"><i class="feather icon-inbox"></i></span>
          <span class="site-mtext"> Visitors Info</span>
        </a>
        <ul class="site-submenu">
          ...
          <li data-username="feedback complaints" class="nav-item @php activeLink(['feedback','feedbacks','complaints']) @endphp">
            <a href="{{ url('cms/feedbacks') }}" class="nav-link">
              <span class="site-mtext">Feedback</span>
            </a>
          </li>
          @endif
    ...
    @if(permission('*.translations.*'))
      <li data-username="translation languages" class="nav-item @php activeLink(['translations']) @endphp">
          <a href="{{ url('cms/translations') }}" class="nav-link">
            <span class="site-micon"><i class="feather icon-file-text"></i></span>
            <span class="site-mtext">Translations</span>
          </a>
      </li>
    @endif

    @if(permission(['*.preferences.*','*.regional_office.*','*.googlemaps.*','*.seo.*']))
      <li data-username="preferences regional office google map seo contacts site settings" class="nav-item site-hasmenu @php activeLink(['preferences','regional_office','seo','googlemaps']) @endphp">
        <a href="javascript:" class="nav-link ">
          <span class="site-micon"><i class="feather icon-settings"></i></span><span class="site-mtext">Site Settings</span></a>
        <ul class="site-submenu">
    ...
    @if(permission(['*.users.*','*.roles.*']))
      <li data-username="role users administration" class="nav-item site-hasmenu @php activeLink(['user-registration-form','roles']) @endphp">
        <a href="javascript:" class="nav-link ">
          <span class="site-micon"><i class="feather icon-users"></i></span><span class="site-mtext">Users & Roles</span></a>
        <ul class="site-submenu">
          @if(permission('*.users.*'))
