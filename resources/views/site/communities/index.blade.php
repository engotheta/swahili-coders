...
@section('title')
{{label('lbl_communities')}}
@endsection

@section('breadcrumb')
<li class="breadcrumb-item" aria-current="page">{{ label('lbl_communities') }}</li>
        ...
        <div class="row">
            <div class="col-lg-12 px-0 my-2 pr-lg-555 page-content">
                ...
                <div class="row mx-0 has-shifting-underline">
					...
					@if($communities->count() || isset($term))
					   <div class="mt-1 col-12">
							<div class="w-100 d-flex justify-content-end  text-center mt-3">
								<form class="form-inline d-flex rounded-medium border-primaryy bg-dark-light overflow-hidden" 
									action="{{url('search-communities')}}" style="width:360px">
									@csrf
							...
							<div class="box-shadow rounded-smallll border-none overflow-hidden  bg-white position-relative mb-4  mt-2 ">
								...
								@foreach ($communities as $key => $entity)
									<div data-name="{{$entity->name}}" class="searchable-record">
										<a href="{{url('communities/'.$category->slug.'/'.$entity->slug)}}" class="row text-hover-primary cursor-pointerrr  hover-bg border-top">
								...
								@if(!$communities->count())
									...
									<a href="{{url('communities/'.$category->id)}}" class="d-block text-center p-3">
