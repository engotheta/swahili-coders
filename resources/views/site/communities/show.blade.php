...
@section('breadcrumb')
<li class="breadcrumb-item"><a href="{{url('communities/'.$community->category->slug)}}">{{$community->category->title}}</a></li>
<li class="breadcrumb-item active" aria-current="page">{!! str_limit(strip_tags($community->name) ,20) !!}</li>
@endsection
                    ...
                    <div class="col-12 py-1 px-0 ">
                        <h5 class="pb-2 position-relative page-title text-uppercase text-primary hover-text-primary"> 
							<span class="d-inline-block py-2 position-relative text-bold text-primary">
							  <span class="px-2222"> {!! $community->name !!}</span>
							</span>
                    ...
                    <div class="col-12 ">
                        {{-- <div class="py-3">
                            <span class="text-muted bold-600 d-inline-block mr-4">
                                <i class="fa fa-calendar-alt mr-1"></i> {!! date('d M, Y', strtotime($community->created_at)) !!}
                            </span>
                        ...
                        <div class="row">
                            @foreach($fields as $key => $field)
                                @if(isset($community[$field]) && trim($community[$field])!=null && 
                                !(substr_count($community[$field],0) == 8) && !($community[$field] == 0) )
                                    <div class="col-md-12 last-no-border py-2 border-bottom-faded ">
                                        <div class="faded text-capitalize">{{str_replace("_", " ", $field)}}</div>
                                        <div class="text-dark mt-0"> {{str_replace("_", " ", $community[$field])}} </div>
                                    </div>
                                @endif
                            @endforeach
                        </div>
